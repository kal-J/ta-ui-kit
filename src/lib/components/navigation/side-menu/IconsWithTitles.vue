                        ...
                        <li class="py-2">
                            <a href="" @click="menu?.onClickHandler"
                                ...
                                <span v-if="menu?.items?.length"
                                    class="absolute left-full top-0 ml-2  px-2 py-1.5 font-medium  opacity-0 hover:z-20 group-hover:opacity-100 group-hover:bg-gray-50 hover:bg-gray-50 w-40">
                                    <nav v-for="item in menu?.items" class="flex flex-col space-y-4">

                                        <a @click="item.onClickHandler" :key="item?.id || uuidv4()" href="#"
                                            class="flex items-center border-l-[3px] border-green-500  px-4 py-3 hover:text-green-700  hover:bg-green-50">
